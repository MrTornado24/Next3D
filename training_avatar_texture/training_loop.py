...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    # Load data.
    images, labels, verts = zip(*[training_set[i] for i in grid_indices])
    if len(images[0].shape) == 4: # select the first frame for every video pair
        images = [image[0] for image in images]
        labels = [label[0] for label in labels]
        verts = [vert[0] for vert in verts]
    return (gw, gh), np.stack(images), np.stack(labels), np.stack(verts)
...
def training_loop(
    ...
    if 'DualLabel' in D_kwargs.class_name:
        D_kwargs['c2_dim'] = 50 # expression label dim
    ...
    if rank == 0:
        ...
        v = torch.empty([batch_gpu, 5023, 3], device=device)
        if training_set.load_lms:
            # load landmarks
            lms = np.loadtxt('data/demo/demo_kpt2d.txt')
            lms = torch.from_numpy(lms).to(device).float().unsqueeze(0)
            v = torch.cat((v, lms.repeat(v.shape[0], 1, 1)), 1)
        img = misc.print_module_summary(G, [z, c_g, v])
        if 'Video' in training_set_kwargs['class_name']:
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c, phase_gen_v in zip(phases, all_gen_z, all_gen_c, all_gen_v):
            ...
            for real_img, real_c, gen_z, gen_c, gen_v in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c, phase_gen_v):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gen_v=gen_v, gain=phase.interval, cur_nimg=cur_nimg)
