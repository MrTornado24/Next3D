...
#----------------------------------------------------------------------------
"""
dual labels uses different mapping networks
"""
@persistence.persistent_class
class DualLabelDualDiscriminator(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        c2_dim,                         # Conditioning label (C2) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
        architecture        = 'resnet', # Architecture: 'orig', 'skip', 'resnet'.
        channel_base        = 32768,    # Overall multiplier for the number of channels.
        ...
        if c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        if c2_dim > 0:
            self.mapping2 = MappingNetwork(z_dim=0, c_dim=c2_dim, w_dim=32, num_ws=None, w_avg_beta=None, **mapping_kwargs)
        self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim + 32, resolution=4, **epilogue_kwargs, **common_kwargs)
        self.register_buffer('resample_filter', upfirdn2d.setup_filter([1,3,3,1]))
        self.disc_c_noise = disc_c_noise

    def forward(self, img, c, update_emas=False, **block_kwargs):
        c1, c2 = c[:, :self.c_dim], c[:, self.c_dim:]
        assert c2.shape[1] == self.c2_dim
        image_raw = filtered_resizing(img['image_raw'], size=img['image'].shape[-1], f=self.resample_filter)
        img = torch.cat([img['image'], image_raw], 1)
        ...
        cmap = None
        if self.c_dim > 0:
            if self.disc_c_noise > 0: c1 += torch.randn_like(c1) * c1.std(0) * self.disc_c_noise
            cmap1 = self.mapping(None, c1)
        if self.c2_dim > 0:
            if self.disc_c_noise > 0: c2 += torch.randn_like(c2) * c2.std(0) * self.disc_c_noise
            cmap2 = self.mapping2(None, c2)

        cmap = torch.cat((cmap1, cmap2), 1)
