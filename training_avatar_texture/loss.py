...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, v, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        gen_output = self.G.synthesis(ws, c, v, neural_rendering_resolution=neural_rendering_resolution, update_emas=update_emas)
    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gen_v, gain, cur_nimg):
        assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth']

        if self.num_frames > 1:
            assert len(real_img.shape) == 5 and real_img.shape[1] == self.num_frames
            gen_z = gen_z.repeat_interleave(real_img.shape[1], dim=0) # [batch_size * num_frames, num_ws]
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, gen_v, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, _gen_ws = self.run_G(gen_z, gen_c, gen_v, swapping_prob=swapping_prob, neural_rendering_resolution=neural_rendering_resolution, update_emas=True)
