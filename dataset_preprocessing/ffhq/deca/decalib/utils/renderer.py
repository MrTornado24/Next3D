...
def compute_rotation_matrix(angles):
    rotXYZ = torch.eye(3).view(1, 3, 3).repeat(3, 1, 1).view(3, 1, 3, 3)
    n_b = angles.size(0)
    ...
    if n_b != 1:
        rotXYZ = rotXYZ.repeat(1, n_b * 3, 1, 1)
    else:
        rotXYZ = rotXYZ.detach().clone()
...
if __name__ == '__main__':
    from util import angle2matrix
    import math
    a = [30/180 * math.pi, 30/180 * math.pi, 30/180 * math.pi,]
    b = [30, 30, 30]
    result_1 = compute_rotation_matrix(torch.tensor(a).unsqueeze(0))
    result_2 = angle2matrix(torch.tensor(b).unsqueeze(0))
