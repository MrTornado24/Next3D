...
def gen_interp_video(G, mp4: str, seeds, verts, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), save_root=None, **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    for frame_idx in tqdm(range(num_keyframes * w_frames)):
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                pitch_range = 0.25
                yaw_range = 0.35
                cam2world_pose = LookAtPoseSampler.sample(3.14/2 + yaw_range * np.sin(2 * 3.14 * frame_idx / (num_keyframes * w_frames // 2)),
                                                        3.14/2 -0.05 + pitch_range * np.cos(2 * 3.14 * frame_idx / (num_keyframes * w_frames // 2)),
                ...
                if entangle == 'conditioning':
                    ...
                    img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], v=verts[0:1], noise_mode='const')[image_mode][0]
                elif entangle == 'both':
                    w_c = G.mapping(z=zs[0:1], c=c[0:1], truncation_psi=psi, truncation_cutoff=truncation_cutoff)
                ...
                #     save_image(img, f"{save_root}/frame_{frame_idx:05d}_seed_{seeds[id_cur]}.png", normalize=True, range=(-1, 1))
...
@click.option('--save_root', help='image save path', type=str, required=False, metavar='DIR', default=None)
...
def generate_images(
    ...
    if interpolate:
        ...
        for seed in seeds:
            ...
            seeds_ = [seed]
            gen_interp_video(G=G, mp4=output, verts=v, bitrate='12M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, save_root=save_root)
